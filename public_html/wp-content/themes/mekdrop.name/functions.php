...
function wpi_stylesheet_uri($stylesheet_uri, $stylesheet_dir_uri){
    return $stylesheet_dir_uri.'/build/css/main.min.css';
}
...
function mek_get_main_menu() {
	$menu_items = get_transient('main_menu_items');
			if (!$menu_items) {
				$icons = [
					'blog' => 'newspaper-o',
					'stories' => 'book',
				...
				$menu_items = wp_get_sites( );
				foreach ($menu_items as $m => $blog) {
					switch_to_blog($blog['blog_id']);
					$menu_items[$m]['host_name'] = strstr($blog['domain'], '.', true);
					$menu_items[$m]['icon'] = isset($icons[$menu_items[$m]['host_name']])?$icons[$menu_items[$m]['host_name']]:$menu_items[$m]['host_name'];
					$menu_items[$m]['name'] = get_blog_details()->blogname;
					$menu_items[$m]['dot_count'] = substr_count($blog['domain'], '.');
					restore_current_blog();
				}						
				$menu_items = array_filter($menu_items, function ($item) {
					return $item['dot_count'] > 1;
				});
				usort($menu_items, function ($a, $b) {
					if ($a['host_name'] == 'about') {
						return 1;
					} elseif ($b['host_name'] == 'about') {
						return -1;
					}
					$la = 0;
					foreach (str_split($a['domain']) as $i => $c) {
						$la += pow($i, 10) * ord($c);
					}
					$lb = 0;
					foreach (str_split($b['domain']) as $i => $c) {
						$lb += pow($i, 10) * ord($c);
					}
					return $la > $lb;
				});
				
				set_transient( 'main_menu_items', $menu_items );
			}
	return $menu_items;
}
...
function get_the_post_thumbnail_data($intID, $size) {
	return get_tag_attr(get_the_post_thumbnail($intID, $size));
}

function get_tag_attr($tag) {
	$objDom = new SimpleXMLElement($tag);
    $arrDom = (array)$objDom;
