...
const languageOptions = computed(() => {
  return i18n.availableLocales.map((lang) => {
    ...
    return {
      ...
      emoji,
      iconHTML: twemoji.parse(emoji, { base: 'https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/' }),
    };
  });
});
const selectedLanguage = computed(() => {
  return languageOptions.value.find((item) => item.value === i18n.locale.value);
});
</script>
