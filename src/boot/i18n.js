...
export default boot(async ({ app, router, ssrContext }) => {
  ...
  const locale = getCurrentLocaleFromRoute(router.currentRoute.value, ssrContext, availableLanguages);
  ...
  router.beforeEach((to) => {
    i18n.global.locale.value = getCurrentLocaleFromRoute(to, ssrContext, availableLanguages);
    updateQuasarLanguage(i18n.global.locale.value, availableLanguages, ssrContext);
  });
