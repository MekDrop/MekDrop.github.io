...
* Grass compression is source-agnostic. It follows the position, orientation, footprint, and ground clearance of animated boot soles and other supported moving physics bodies. Active contacts part and flatten the 3D canopy; released impressions recover smoothly within about one second. Airborne bodies must not create new impressions, and contact must not affect grass at another elevation. The underlying terrain texture stays still, without painted footprints, brushing, or wind effects.
...
* Exposed vertical grass cliffs may rarely receive seed-deterministic hanging vine clusters, capped at four clusters per map. Each cluster is anchored to one complete cliff face, stays between that face's upper and lower terrain elevations, and must not cross a river or waterfall face. Vines are non-colliding scenery and do not change tile ownership, buildability, or terrain geometry.
* The grass carpet is visual terrain detail: it does not change tile ownership, elevation, collision, buildability, or collectible placement. Solid river source covers and the grass-topped reserved ground beneath ordinary bridges carry it, but exposed water, walkable path surfaces, gates, castle foundations, and dirt-only grade-separated fill must remain clear. Excavation clears blades around the hole and keeps filled soil exposed.
...
* Water uses an opaque, illustrated turquoise/mint palette with soft off-white foam strokes, restrained view-dependent procedural sky reflection, and small sun highlights. Palette colors are converted to linear space so the renderer does not wash them out. Horizontal paint coordinates are evaluated from world position and a nearest-sampled route metadata texture, carrying foam through orthogonal bends without interpolated UV seam lines at tile boundaries. Surface cells retain their complete half-height volume, reconcile tiny bridge-clearance height differences at shared corners, and use one shared wave field across bends. Each generated river combines its horizontal volumes, internal cascades, and terminal waterfall into one mesh and material group. The waterfall lip begins at the exact full-width downstream cell boundary, so the join does not depend on draw order or depth bias. Surface and falling shader regions meet directly at that boundary without an interpolated connector strip. Water rendering uses its own vertex, color, and opacity shaders; lava retains its separate molten shading and geometry.
* Each water cascade is rebuilt as a continuous rounded spillway and a thin falling curtain with connected rear and side faces. Both faces descend without an upward-folding rear lip, share coherent motion, and extend below the receiving surface; the rear catches up to the front below the upstream riverbed so the side does not remain a stretched panel. Side faces use depth-separated foam strands advected along the spillway arc: water travels forward out of the river, bends gradually downward, and joins the vertical curtain. Side paint must not scroll straight down through the still-horizontal interior. Short-lived droplets begin halfway down the curtain and peel away at multiple heights in its lower half, with small upward splashes at internal landings; spray occupies the open pocket between the upstream cliff and the rear of the falling curtain with restrained drift, spans its width rather than gathering in a central plume, has visible depth from side views, and stays within the channel. Droplets remain small, softly edged, and lightly stretched so front views do not fill with bright confetti. Drops up to and including four cubes keep the full upstream width throughout; only taller falls gradually narrow through their falling body. Irregular aqua channels and torn foam patches split, join, and end at different heights as they move down the falling sheet; periodic parallel stripes are avoided. Internal cascades finish on their receiving water surface. Exposed riverbed cliffs behind cascades use the adjacent bank stone-patterned soil texture in tile-height layers, rather than one stretched earth face. The same stone texture covers their exposed ledges and every riverbed floor, including zero-elevation cells and river cells beneath bridges. The falling sheet drives a localized churning footprint that interrupts ordinary surface ribbons, spreads water and broken foam outward from the impact (including backwash behind the curtain), turns that current away from closed banks into the receiving route (including L-turns), and fades downstream. Both color and foam follow this local current instead of retaining ordinary route scrolling beneath a distortion overlay. Broken backwash crests reach every closed landing bank, including the far wall and inner corner of an L-turn, and roll back into the current; open downstream joins must not acquire a wall-foam seam. Impact foam is shaded directly into the water instead of overlapping strips, and lower landing cells must not restart the source-pressure effect. Terminal falls extend to the existing generated bottom elevation, taper into an irregular open tail, and fade into animated mist and spray. Rendering does not change the generated river cells, elevations, cascade endpoints, flow directions, bridge clearances, or hero-current behavior.
* Every non-bridge horizontal segment materializes full-height earth banks on both lateral sides, including where the original island mask contained a notch. A terminal waterfall also materializes in-bounds, full-height earth shoulder cells immediately beyond both sides of its lip, leaving only the center outflow open. Every source is enclosed by a complete 3×3 ring of raised earth, excluding only cells occupied by its downstream river route. Its first water tile stays free of stones and is covered by a full-width, full-length, one-third-cube-high grass-and-earth cap, flush with the surrounding ground. Water emerges from beneath the cap at its downstream edge, without a vertical water jet, water plume, portal ring, or side-facing opening. Several fast, overlapping pressure waves continuously disturb the water inside the hidden, covered source tile. The resulting cel-painted surges reach toward the top of the opening across its complete width, flow through it, and lose height across the first exposed cell. A smaller broken foam line churns at the outlet between surges. Source pressure displacement must remain continuous across a waterfall lip immediately beside the source, including its side faces, and fade down the spillway. Source motion uses one world-space field through straight and L-shaped outlets, vanishing across the complete downstream edge before the next surface cell. The source effect belongs to the water surface rather than spawning mist clouds inside or above the source tile, and it must read as newly produced water rather than translated texture or a single repeating wave. Only the intended downstream opening and waterfall face remain open.
* Horizontal river cells may contain a small deterministic scattering drawn from multiple authored low-poly stone shapes using natural cool-slate, warm-gray, and dark-slate materials. Stone width, height, silhouette, rotation, and burial depth vary broadly, but every stone begins at or below the riverbed so it remains physically rooted rather than floating. Stones stay clear of bridges, waterfall source and landing cells, and cascade impact foam. Exposed water stones produce contact foam only against their upstream faces, irregular side trails drifting downstream, and a small rising spray/mist emitter; complete foam rings and repeated arrow-shaped wake bands are forbidden. Moving surface patterns split around exposed stones and rejoin downstream, with faster flow along their flanks while bank foam stays anchored. Deflection affects moving foam ribbons without magnifying the broad water-color wash, and contact mist stays small enough to avoid a cloudy lens around the stone; fully submerged stones and lava do not produce this water effect.
* A hero caught in the current sinks and rises unevenly while retaining the existing current speed, escape, and waterfall behavior. Nearby open water displays a faint, rippled reflection captured from the actual animated head. This small offscreen capture is disabled outside drowning and beneath bridges. Derivatives and implicit texture samples in water shaders must remain in uniform control flow for WebGPU.
